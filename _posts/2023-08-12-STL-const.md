...
tags: [C++, STL, Standard Template Library, const, 상수객체, 상수메서드]
---
...
### 함수의 매개변수
매개변수를 `참조`로 받는 것은 `Call by reference` 로 `복사의 오버헤드 없이 변수를 참조`하고, 해당 변수를 함수 내에서 변경하지 않겠다는 의미입니다. 또한, `복사 생성자가 호출되지 않습니다.`
